...
class App extends Component {
  ...
  tick = () => {
    ...
    if (timer !== 0) {
      this.setState(each => ({timer: each.timer - 1}))
    } else {
  ...
  changeImage = thumbnailUrl => {
    const {displayImage, show} = this.state
  ...
  render() {
    const {score, timer, show, displayImage, activeTab} = this.state
      ...
      <div className="main-container">
        <navbar className="nav-container">
          ...
          <ul className="nav-list">
            <li className="nav-li">
              <p className="nav-para">score: </p>
              <p className="nav-par">{score}</p>
            </li>
            <li className="nav-li">
              <img
                ...
                className="timer"
              />
              <p className="nav-par">{timer} </p>
              <p className="nav-par"> sec</p>
            </li>
          </ul>
        </navbar>
        {!show && (
          <div className="image-container">
            <img src={displayImage} alt="match" className="displayImage" />
          </div>
        )}
        {!show && (
          <ul className="tab-container">
            {tabsList.map(each => (
              <li className="tab-list" key={each.tabId}>
                <button
                  type="button"
                  className="tab-btn"
                  onClick={() => this.tabChange(each.tabId)}
                >
                  {each.displayText}
                </button>
              </li>
        ...
        {!show && (
          <ul className="tn-container">
            {filterImagesList.map(each => (
              <li className="tn-list" key={each.id}>
                <button
                  type="button"
                  className="tab-btn"
                  onClick={() => this.changeImage(each.thumbnailUrl)}
                >
                  <img
        ...
        {show && (
          <div className="gameover">
            <img
              ...
              className="trophy"
            />
            <p className="trophypara"> YOUR SCORE</p>
            <p className="trophypara">{score}</p>
            <button
              type="button"
